...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        setTimeout(() => {
            ...
            setInterval(refreshBusPositions, GPS_POLL_MS);
        }, 1000);
...
async function refreshBusPositions() {
    try {
        gpsLocations = await fetchJSON(`${API}/gps`);
    } catch (error) {
        console.error("Could not refresh bus positions:", error);
        return;
    }
...
function renderBusMarkers() {
    ...
    // Drop markers for buses that no longer appear in the feed
    busMarkersByBusId.forEach((marker, busId) => {
        if (!latestByBus.has(busId)) {
            marker.remove();
            busMarkersByBusId.delete(busId);
        }
    });

    latestByBus.forEach((location, busId) => {
        if (!hasValidCoordinates(location)) return;

        const latLng = [Number(location.latitude), Number(location.longitude)];
        const popupHTML = `
            <strong>${location.bus_number || `Bus ${busId}`}</strong><br>
            Speed: ${location.speed || 0} km/h<br>
            Updated: ${location.recorded_at || "Unknown"}
        `;

        const existingMarker = busMarkersByBusId.get(busId);

        if (existingMarker) {
            // Update in place (rather than remove + recreate) so the
            // CSS transition on .bus-map-icon can animate the move.
        ...
        busMarkersByBusId.set(busId, marker);
    });
}

function updateNextArrival() {
    const nextBusOutput = document.getElementById("nextBusOutput");
    if (!nextBusOutput) return;

    const destSelect = document.getElementById("destSelect");
    const destinationStop = destSelect
        ? stops.find((stop) => String(stop.stop_id) === String(destSelect.value))
        : null;

    if (!destinationStop || !hasValidCoordinates(destinationStop)) {
        nextBusOutput.textContent = "--";
        return;
    }

    const latestByBus = new Map();
    gpsLocations.forEach((location) => {
        if (!latestByBus.has(location.bus_id)) {
    ...
    if (!closest) {
        nextBusOutput.textContent = "No buses tracked yet";
        return;
    }

    // Rough estimate: straight-line distance over last known (or typical) speed.
    const speedKmh = Number(closest.location.speed) > 0 ? Number(closest.location.speed) : 30;
    const etaMinutes = Math.max(1, Math.ceil((closest.distanceKm / speedKmh) * 60));
    const busLabel = closest.location.bus_number || `Bus ${closest.location.bus_id}`;

    nextBusOutput.textContent = `~${etaMinutes} min (${busLabel}, ${closest.distanceKm.toFixed(1)} km away)`;
}
